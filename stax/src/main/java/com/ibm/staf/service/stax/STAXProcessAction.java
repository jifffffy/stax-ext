                                          ...
                                          STAXTimedEventListener
{
    ...
    public void execute(STAXThread thread)
    {
        synchronized (this)
        {
            if (fState == INIT)
            {
                ...
                if (fThread.getJob().getSTAX().getDebugProcess())
                {
                    STAX.logToJVMLog(
                        "Debug", fThread,   
                        "key=" + fLocation +
                ...
                if (false && fThread.getJob().getSTAX().getDebugProcess())
                {
                    STAX.logToJVMLog(
                        "Debug", fThread,                        
                        "key=" + fLocation +
            ...
            else if (fThread.getJob().getSTAX().getDebugProcess())
            {
                // Should not get here
                                             ...
                                             STAXCondition cond)
    {
        synchronized (this)
        {
            if (thread.getJob().getSTAX().getDebugProcess())
            {
                STAX.logToJVMLog(
                    "Debug", thread,                      
                    "key=" + fLocation + ":" + fProcessHandle +
        ...
        STAFResult result)
    {
        ...
        // Remove the timeout event from the STAXTimedEventQueue
        
        fThread.getJob().getTimedEventQueue().removeTimedEvent(fTimedEvent);
