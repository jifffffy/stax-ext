                                      ...
                                      STAXThreadCompleteListener
{
    ...
    /**
     * Gets the Invalid Log Level Action option specified for the sub-job.
     * @return the Invalid Log Level Action option specified for the sub-job
     */
    public String getInvalidLogLevelAction() { return fInvalidLogLevelAction; }

    /**
     * Sets the Invalid Log Level Action option.
     * It defaults to the same value specified for its parent job.
     * @param  invalidLogLevelAction  the invalidLogLevelAction option for the
     * sub-job
     */
    public void setInvalidLogLevelAction(String invalidLogLevelAction)
    {
        fInvalidLogLevelAction = invalidLogLevelAction;
        fUnevalInvalidLogLevelAction = invalidLogLevelAction;
    }
    ...
    public synchronized void execute(STAXThread thread)
    {
        if (fState == INIT)
        {
            if (sDebug)
                STAX.logToJVMLog(
                    "Debug", thread,
                    "STAXJobAction::execute(): fState=INIT");
            ...
            try
            {
                ...
                // Evaluate the invalidloglevelaction attribute

                evalElem = "job";
                evalAttr = "invalidloglevelaction";

                if (fUnevalInvalidLogLevelAction.equals(""))
                {
                    // Default to parent job's option if not specified
                    fInvalidLogLevelAction =
                        STAXLogAction.getInvalidLogLevelActionAsString(
                            thread.getJob().getInvalidLogLevelAction());
                }
                else
                {
                    fInvalidLogLevelAction = thread.pyStringEval(
                        fUnevalInvalidLogLevelAction);

                    if (fInvalidLogLevelAction.equalsIgnoreCase("parent"))
                    {
                        fInvalidLogLevelAction =
                            STAXLogAction.getInvalidLogLevelActionAsString(
                                thread.getJob().getInvalidLogLevelAction());
                    }
                    else if (fInvalidLogLevelAction.equalsIgnoreCase("default"))
                    {
                        fInvalidLogLevelAction =
                            STAXLogAction.getInvalidLogLevelActionAsString(
                                thread.getJob().getSTAX().getInvalidLogLevelAction());
                    }
                }

                request.append(" INVALIDLOGLEVELACTION ").append(
                    STAFUtil.wrapData(fInvalidLogLevelAction));
            ...
            else
            {
                ...
                if (STAFMarshallingContext.isMarshalledData(fSubmitResult))
                {
                    ...
                    // and Error Message.  If so, assign these values to
                    ...
                    catch (STAXPythonEvaluationException e)
                    {
                        ...
                        STAX.logToJVMLog(
                            "Error", fThread,
                            "STAXJobAction::execute(): " +
                            " Error assigning STAXSubJobID/STAFResult\n" +
                            e.toString());
                        fThread.pySetVar("STAFResult", fSubmitResult);
                        fThread.pySetVar("STAXSubJobID", new Integer(0));
                    }
            ...
            subJobMap.put("invalidloglevelaction", fInvalidLogLevelAction);
    ...
    public STAXAction cloneAction()
    {
        ...
        clone.fUnevalInvalidLogLevelAction = fUnevalInvalidLogLevelAction;
    ...
    private String fInvalidLogLevelAction = new String();
    ...
    private String fUnevalInvalidLogLevelAction = new String();
